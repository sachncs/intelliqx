...
- **Context**: The platform needs a single LLM client surface that
  ...
  The two registered backends today are:
  - **Fake** — deterministic hash-based responses for tests and
...
- **Decision**: Standardise on
  ...
  (pinned in `libs/intelliqx-llm/pyproject.toml`).
  The `intelliqx_llm.client.LLMClient` interface stays the platform
  contract; the MiniMax adapter wraps `litellm.acompletion` /
  `litellm.aembedding` behind it.
- **Consequences**:
  - Pros: one set of retry / token-count / streaming semantics
    across every provider.
  - Pros: the MiniMax adapter falls back to the deterministic
    helpers in `intelliqx_llm.client` when the SDK is missing or
    credentials are absent, so the rest of the platform keeps
    running in CI on a laptop with no API keys.
  - Cons: litellm is a large dependency (it pulls in tiktoken,
    tokenizers, jinja2, openai, etc.).
