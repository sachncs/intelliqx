...
lives in feature-specific libs (``aqip-events.modal``,
``aqip-storage.modal``, …). The adapter exposes the resolved config
...
class ModalAdapter(CloudAdapter):
    ...
    one to the caller), so :class:`~aqip_portability.config.CloudConfig`'s
