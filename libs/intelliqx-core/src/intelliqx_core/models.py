...
class AgentCategory(str, Enum):
    ...
    Categories are descriptive — every ``RoleSpec`` row in
    :mod:`agents.ai.roles` carries one of these values, and the
    AgentCatalog groups rows by category. The enum replaces the
    older numeric ``tier`` field. Order in the enum mirrors the
    platform's dependency graph: coordination orchestrates
    everything, then intelligence decides, execution runs, and
    governance observes.
    """
