...
class Counter:
    ...
    def __init__(self, name: str, description: str = "") -> None:
        ...
        self.__values: dict[tuple[tuple[str, str], ...], float] = defaultdict(float)
    ...
    def value(self, **labels: Any) -> float:
        ...
        """
        key = tuple(sorted(labels.items()))
        return self.__values.get(key, 0.0)
...
class Gauge:
    ...
    def __init__(self, name: str, description: str = "") -> None:
        self.name = name
        self.description = description
        self.__values: dict[tuple[tuple[str, str], ...], float] = {}
        self.__lock = threading.Lock()
...
class Histogram:
    ...
    def __init__(self, name: str, description: str = "") -> None:
        self.name = name
        self.description = description
        self.__samples: dict[tuple[tuple[str, str], ...], list[float]] = defaultdict(list)
        self.__lock = threading.Lock()
    ...
    def observe(self, value: float, **labels: Any) -> None:
        ...
        key = tuple(sorted(labels.items()))
        with self.__lock:
            self.__samples[key].append(value)
    ...
    def snapshot(self) -> dict[str, dict[str, float]]:
        ...
        with self.__lock:
            for k, samples in self.__samples.items():
                if not samples:
                    continue
...
class MetricsRegistry:
    ...
    def counter(self, name: str, description: str = "") -> Counter:
        ...
        with self.__lock:
            if name not in self.__counters:
                self.__counters[name] = Counter(name, description)
    ...
    def gauge(self, name: str, description: str = "") -> Gauge:
        ...
        with self.__lock:
            if name not in self.__gauges:
                self.__gauges[name] = Gauge(name, description)
    ...
    def histogram(self, name: str, description: str = "") -> Histogram:
        ...
        with self.__lock:
            if name not in self.__histograms:
                self.__histograms[name] = Histogram(name, description)
            return self.__histograms[name]
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        """
        return {
            "counters": {n: c.snapshot() for n, c in self.__counters.items()},
            "gauges": {n: g.snapshot() for n, g in self.__gauges.items()},
            "histograms": {n: h.snapshot() for n, h in self.__histograms.items()},
        }
