...
class CallGraphBuilder(LayerBuilder):
    ...
    def build(self, parsed_data: dict[str, Any], existing: SGIRGraph | None = None) -> SGIRGraph:
        ...
        return SGIRGraph(layer=GraphLayer.CALL, nodes=nodes, edges=edges, metadata=metadata)


def find_call_target(
    call_name: str, caller: Any, entities: list[Any], node_ids: set[str]
) -> str | None:
    for entity in entities:
        if entity.name == call_name:
            ...
            if nid in node_ids:
                return nid
    parent_id = (
        make_node_id(caller.file_path, f"{caller.parent}.{call_name}") if caller.parent else None
    )
