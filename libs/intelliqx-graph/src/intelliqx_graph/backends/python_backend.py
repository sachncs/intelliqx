...
class PythonBackend(CodeBackend):
    ...
    def generate(self, graph: SoftwareGraph) -> dict[str, str]:
        ...
        for module in module_nodes:
            ...
            module_classes = [
                c for c in class_nodes if self._is_child_of(c, module, graph)
            ]
            module_functions = [
                f for f in function_nodes if self._is_child_of(f, module, graph)
            ]
            module_methods = [
                m for m in method_nodes if self._is_child_of(m, module, graph)
            ]
            module_datamodels = [
                d for d in datamodel_nodes if self._is_child_of(d, module, graph)
            ]
        ...
        for package in package_nodes:
            dir_path = self._node_to_dir_path(package)
            init_path = f"{dir_path}/__init__.py"
            if init_path not in files:
                files[init_path] = self._generate_package_init(package)

        orphans = [
            n for n in class_nodes + function_nodes + datamodel_nodes
            if not any(
                self._is_child_of(n, m, graph) for m in module_nodes
            )
        ]
        if orphans:
            file_path = f"{graph.repository.name.replace(' ', '_').lower()}.py"
    ...
    def _node_to_dir_path(self, node: SGIRNode) -> str:
        if node.source_location:
    ...
    def _generate_single_file(
        ...
        for dm in datamodels:
            sections.append(self._generate_datamodel(dm))
        for cls in classes:
    ...
    def _generate_module_file(
        ...
        imports = self._collect_imports_for_node(module, graph)
        if imports:
            sections.append("\n".join(imports))
        for dm in datamodels:
            sections.append(self._generate_datamodel(dm))
    ...
    def _generate_package_init(self, package: SGIRNode) -> str:
        parts: list[str] = []
        if package.purpose:
    ...
    def _generate_class(
        ...
        bases = self._get_inheritance(cls, graph)
        base_str = f"({', '.join(bases)})" if bases else ""
        parts = [f"class {self._safe_name(cls.name)}{base_str}:"]
        if cls.purpose:
            parts.append(indent(f'"""{cls.purpose}"""', "    "))
        class_methods = [
            m for m in all_methods if self._is_child_of(m, cls, graph)
        ]
    ...
    def _generate_method(self, method: SGIRNode) -> str:
        params = self._inputs_to_params(method)
        ret = self._outputs_to_return_type(method)
        sig = f"def {self._safe_name(method.name)}({params}){ret}:"
        body = self._generate_body(method)
        return indent(f"{sig}\n{body}", "    ")

    def _generate_function(self, func: SGIRNode) -> str:
        params = self._inputs_to_params(func)
        ret = self._outputs_to_return_type(func)
        sig = f"def {self._safe_name(func.name)}({params}){ret}:"
        body = self._generate_body(func)
        return f"{sig}\n{body}"

    def _generate_datamodel(self, dm: SGIRNode) -> str:
        fields = self._inputs_to_fields(dm)
        parts = [f"class {self._safe_name(dm.name)}:"]
        if dm.purpose:
    ...
    def _generate_body(self, node: SGIRNode) -> str:
        lines = ["pass"]
        if node.purpose:
            lines = [f'"""{node.purpose}"""']
        return indent("\n".join(lines), "    ")

    def _inputs_to_params(self, node: SGIRNode) -> str:
        params = ["self"] if node.node_type == NodeType.METHOD else []
        for inp in node.inputs:
            parts = inp.split(":")
            name = self._safe_name(parts[0].strip())
            if len(parts) > 1:
    ...
    def _inputs_to_fields(self, node: SGIRNode) -> list[str]:
        fields = []
        for inp in node.inputs:
            parts = inp.split(":")
            name = self._safe_name(parts[0].strip())
    ...
    def _outputs_to_return_type(self, node: SGIRNode) -> str:
        if not node.outputs:
            return ""
    ...
    def _get_inheritance(self, cls: SGIRNode, graph: SoftwareGraph) -> list[str]:
        bases: list[str] = []
        for layer_graph in graph.layers.values():
            for edge in layer_graph.edges:
                if edge.target == cls.id and edge.source in (
                    ...
                    if source_node and source_node.node_type == NodeType.CLASS:
                        bases.append(self._safe_name(source_node.name))
        return bases
    ...
    def _collect_imports_for_node(self, node: SGIRNode, graph: SoftwareGraph) -> list[str]:
