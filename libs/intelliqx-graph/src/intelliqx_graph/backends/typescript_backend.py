...
class TypeScriptBackend(CodeBackend):
    ...
    def generate(self, graph: SoftwareGraph) -> dict[str, str]:
        ...
        if not module_nodes and not package_nodes:
            file_path = f"src/{self.to_kebab_case(graph.repository.name)}.ts"
    ...
    def node_to_file_path(self, node: SGIRNode) -> str:
        ...
        name = self.to_kebab_case(node.name)
    ...
    def generate_single_file(
        ...
        for dm in datamodels:
            sections.append(self.generate_interface(dm))
        for cls in classes:
    ...
    def generate_module_file(
        ...
        for func in functions:
            if func.node_type != NodeType.METHOD:
                sections.append(self.generate_exported_function(func))
        return self.format_code("\n\n".join(sections))

    def generate_interface(self, node: SGIRNode) -> str:
        type_name = self.to_pascal_case(node.name)
        parts = [f"export interface {type_name} {{"]
        for inp in node.inputs:
            field = self.to_ts_field(inp)
            if field:
    ...
    def generate_class(
        ...
        class_methods = [m for m in all_methods if self.is_child_of(m, cls, graph)]
        if class_methods:
            parts.append("")
            for method in class_methods:
                parts.append(self.generate_class_method(method))
                parts.append("")
        parts.append("}")
        return "\n".join(parts)

    def generate_class_method(self, method: SGIRNode) -> str:
        method_name = self.to_camel_case(method.name)
        params = self.to_ts_params(method)
        returns = self.to_ts_return_type(method)
        sig = f"  {method_name}({params}){returns} {{"
        body = indent(self.generate_body_ts(method), "    ")
        return f"{sig}\n{body}\n  }}"

    def generate_function(self, func: SGIRNode) -> str:
        func_name = self.to_camel_case(func.name)
        params = self.to_ts_params(func)
        returns = self.to_ts_return_type(func)
        sig = f"function {func_name}({params}){returns} {{"
        body = self.generate_body_ts(func)
        return f"{sig}\n{body}\n}}"

    def generate_exported_function(self, func: SGIRNode) -> str:
        func_name = self.to_camel_case(func.name)
        params = self.to_ts_params(func)
        returns = self.to_ts_return_type(func)
    ...
    def to_ts_params(self, node: SGIRNode) -> str:
        params = []
        for inp in node.inputs:
            parts = inp.split(":")
            name = parts[0].strip().replace("-", "_")
            if len(parts) > 1:
                ts_type = self.map_ts_type(parts[1].strip())
                params.append(f"{name}: {ts_type}")
            else:
                params.append(f"{name}: any")
        return ", ".join(params)

    def to_ts_return_type(self, node: SGIRNode) -> str:
        if not node.outputs:
            return "void"
        if len(node.outputs) == 1:
            return self.map_ts_type(node.outputs[0].strip())
        types = " | ".join(self.map_ts_type(o.strip()) for o in node.outputs)
        return f"[{types}]"

    def to_ts_field(self, inp: str) -> str:
        parts = inp.split(":")
        name = parts[0].strip().replace("-", "_")
        if len(parts) > 1:
            ts_type = self.map_ts_type(parts[1].strip())
    ...
    def map_ts_type(self, type_hint: str) -> str:
        mapping = {
            "str": "string",
            "string": "string",
            "int": "number",
            ...
            for edge in layer_graph.edges:
                if edge.edge_type.value == "import":
                    ...
                    if source_node and source_node.name not in seen:
                        ...
                        imports.append(f'import {{ {self.to_pascal_case(source_node.name)} }} from "{source_node.name}";')
        return imports

    def generate_body_ts(self, node: SGIRNode) -> str:
