...
class CostOptimizationAgent(AgentBase):
    ...
    INPUT_MODEL = CostOptimizationInput
    OUTPUT_MODEL = CostOptimizationOutput

    @traced_agent("cost_optimization")
    async def run(self, ctx: AgentContext, input: CostOptimizationInput) -> CostOptimizationOutput:
        ...
        failed = sum(v for d in counters.values() for v in d.values() if "fail" in str(d).lower())
        if failed > 10:
            recs.append(
                CostRecommendation(
                    target="ci",
        ...
        return CostOptimizationOutput(
            recommendations=recs, estimated_total_savings_usd=total_savings
        )
