...
from agents.intelligence.models import CodeImpactGraph
        ...
        for e in edges:
            for cp in changed_set:
                if e.dst in path_to_id and path_to_id[e.dst].endswith(cp.split("/")[-1]):
                    impact_path: str | None = next(
                        (p for p, pid in path_to_id.items() if pid == e.src), None
                    )
                    if impact_path:
                        affected.add(impact_path)
        if not changed_set:
