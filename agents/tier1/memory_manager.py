...
* **Working memory** — fast, TTL-bounded. Backed by
  :class:`aqip_state.store.StateStore` (Redis in production). Key
  shape: ``{tenant_id}:working:{key}``.
* **Episodic / semantic / code memory** — durable. Backed by
  :class:`aqip_storage.store.ObjectStore` (S3 / GCS / Modal Volume in
...
class MemoryManagerAgent(AgentBase):
    ...
    async def invoke(self, request):  # type: ignore[no-untyped-def]
        ...
        model = _MODEL_BY_OP.get(op, MemoryPut)
    ...
    async def run(self, ctx: AgentContext, input: MemoryPut) -> MemoryOutput:
        # ``input`` is typed as MemoryPut (the most common case)
        # but the actual instance may be any of the five operation
        # models — we branch on isinstance.
        tenant_id = ctx.tenant.tenant_id
        if isinstance(input, MemoryPut):
    ...
    async def _put(self, tenant_id: str, op: MemoryPut) -> MemoryOutput:
