...
class ToolManagerInput(BaseModel):
...
class ToolManagerOutput(BaseModel):
...
def default_tool_manager() -> ToolManager:
    ...
    if not mgr.registry.list_tools():
        mgr.register_tool(
            ToolDefinition(
                name="github.issue",
...
class ToolManagerAgent(AgentBase):
    ...
    INPUT_MODEL = ToolManagerInput
    OUTPUT_MODEL = ToolManagerOutput

    @traced_agent("tool_manager")
    async def run(self, ctx: AgentContext, input: ToolManagerInput) -> ToolManagerOutput:
        mgr = default_tool_manager()
        res = await mgr.invoke(input.tool, payload=input.payload)
        return ToolManagerOutput(
            tool=res.tool, status=res.status, output=res.output, error=res.error
        )
