...
class AccessibilityAgent(AgentBase):
    ...
    INPUT_MODEL = A11yInput
    OUTPUT_MODEL = A11yOutput

    @traced_agent("accessibility")
    async def run(self, ctx: AgentContext, input: A11yInput) -> A11yOutput:
        ...
        # Rule: button text
        for m in re.finditer(r"<button\b([^>]*)>(.*?)</button>", input.dom_html, re.IGNORECASE | re.DOTALL):
        ...
        return A11yOutput(issues=issues, passed=not issues)
