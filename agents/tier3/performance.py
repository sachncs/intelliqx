...
class PerformanceAgent(AgentBase):
    ...
    INPUT_MODEL = PerfInput
    OUTPUT_MODEL = PerfOutput
    ...
    async def run(self, ctx: AgentContext, input: PerfInput) -> PerfOutput:
        timings: list[float] = []
        errors = 0
        total = 0
        ...
        async def worker() -> None:
            ...
            # Per-worker client would be more isolated; we share
            # one to reuse the connection pool. The trade-off is
            # that errors on one worker affect the others'
            # client state — acceptable for short-lived tests.
        ...
        # Percentiles: sort once, then index by the nearest-rank
        # formula. For small samples, the percentile is just the
        # last sample.
        timings.sort()
        n = len(timings)
